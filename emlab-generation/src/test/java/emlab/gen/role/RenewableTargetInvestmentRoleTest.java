...
public class RenewableTargetInvestmentRoleTest {
    ...
    public void testActMethod() {
        ...
        assertEquals("Test wind capacity after investment in year 0", 300, powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, pv, 0), 0.1);

        genericInvestmentRole.act(rti);

        assertEquals("Test wind capacity after investment in year 3", 400, powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, wind, 2), 0.1);
        // Expected value: (Start + ExpectedTimeForWind*Increment: 400 + 3 * 100
        // = 700, but 800 because of plant size.
        assertEquals("Test wind capacity after investment in year 3", 700, powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, wind, 3), 0.1);

        assertEquals("Test wind capacity after investment in year 0", 300, powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, pv, 0), 0.1);
        // 450 is closer to the target of 500 than 600.
        assertEquals("Test wind capacity after investment in year 0", 500, powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, pv, 1), 0.1);
    }
