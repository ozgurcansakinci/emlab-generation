...
public class DetermineAnnualDemandGrowthRole extends AbstractMarketRole<ElectricitySpotMarket>
    ...
    public void act(ElectricitySpotMarket market) {
        ...
        if (getCurrentTick() == 0) {
            ...
            DoubleMatrix1D hourlyArray = new DenseDoubleMatrix1D(market.getYearlySegmentLoad()
                    .getHourlyInElasticCurrentDemandForYearlySegment().getHourlyArray(getCurrentTick()));
            ...
            // hourlyArray.assign(growthRate,Functions.mult);
            hourlyArray.assign(growthFactors, Functions.mult);
            logger.warn("hourly array:" + hourlyArray.toArray()[0]);
            // market.getYearlySegmentLoad().setHourlyInElasticCurrentDemandForYearlySegment();
            // market.getYearlySegmentLoad().getHourlyInElasticCurrentDemandForYearlySegment()
            // .setHourlyArray(hourlyArray.toArray(), 0);
            ...
            // market.getYearlySegmentLoad().makeDemandGrow(hourlyArray.toArray());
            ...
            if (market.isDailyDemandResponseImplemented()) {
                DoubleMatrix1D dailyArray = new DenseDoubleMatrix1D(market.getYearlySegmentLoad()
                        .getDailyElasticCurrentDemandForYearlySegment().getHourlyArray(getCurrentTick()));
                DoubleMatrix1D dailyGrowth = dailyArray.copy();
                dailyGrowth.assign(growthRate);
                dailyArray.assign(dailyGrowth, Functions.mult);
                // market.getYearlySegmentLoad().getDailyElasticCurrentDemandForYearlySegment()
