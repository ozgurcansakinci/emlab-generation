 ...
 */
@RoleComponent
public class SubmitLongTermElectricityContractsRole extends AbstractEnergyProducerRole implements Role<EnergyProducer> {
            ...
            if (getCurrentTick() > 0 && plant.isWithinTechnicalLifetime(getCurrentTick())
                ...
                for (LongTermContractType type : reps.genericRepository.findAll(LongTermContractType.class)) {
                    ...
                    for (LongTermContractDuration duration : reps.genericRepository
                        ...
                        // contracts in the
                        LongTermContractOffer offer = reps.contractRepository.submitLongTermContractOfferForElectricity(
                                producer, plant, zone, thisPrice, capacity, type, getCurrentTick(), duration, mainFuel,
                                passThroughFuel, passThroughCO2, fuelPriceStart, co2PriceStart);
    ...
    /**
     * Determines the average electricity price weighted by hours of segments of
     * a previous number of years.
     * 
     * @param hours
     *            the total hours in these segments
     * @param type
     *            the contract type
     * @param the
     *            horizon to look back
     * @return the average price
     */
    private double determineAverageElectricityPrice(double hours, int horizon, LongTermContractType type) {
        ...
        for (int i = -horizon; i <= -1; i++) {
            ...
            for (Segment s : type.getSegments()) {
                ...
                SegmentClearingPoint point = (SegmentClearingPoint) reps.clearingPointRepositoryOld
                        .findClearingPointForSegmentAndTime(s, getCurrentTick() + i, false);

                // If there is a price, add it multiplied to the number of hours
                // to the total.
                if (point != null) {
    ...
    private double determineTotalAverageCost(PowerPlant plant, double hours) {
        ...
        double fixedcapitalCost = plant.getActualInvestedCapital() // Doesn't
                                                                   // consider
                                                                   // the cost
                                                                   // of
                                                                   // capital,
        ...
        return determineFuelCost(plant) + determineCO2Cost(plant) + ((fixedOMCost + fixedcapitalCost) / hours);
    }
