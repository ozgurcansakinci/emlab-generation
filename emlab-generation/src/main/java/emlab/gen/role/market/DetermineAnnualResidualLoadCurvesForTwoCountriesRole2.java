 ...
 */
@RoleComponent
public class DetermineAnnualResidualLoadCurvesForTwoCountriesRole2 extends AbstractRole<DecarbonizationModel>
    ...
    public void act(DecarbonizationModel model) {
        ...
        // Map<Zone, Map<PowerGridNode, Map<PowerGeneratingTechnology,
        // Integer>>> TECHNOLOGYLOADFACTORSFORZONEANDNODE = new HashMap<Zone,
        // Map<PowerGridNode, Map<PowerGeneratingTechnology, Integer>>>();
        // for (Zone zone : zoneList) {
        // Map<PowerGridNode, Map<PowerGeneratingTechnology, Integer>>
        // NODETOTECHNOLOGY = new HashMap<PowerGridNode,
        // Map<PowerGeneratingTechnology, Integer>>();
        // for (PowerGridNode node : zoneToNodeList.get(zone)) {
        // Map<PowerGeneratingTechnology, Integer> technologyToColumn = new
        // HashMap<PowerGeneratingTechnology, Integer>();
        ...
        for (Zone zone : zoneList) {
            ...
            DoubleMatrix1D hourlyArray = new DenseDoubleMatrix1D(info.getMarketDemand());
        ...
        for (Zone zone : zoneList) {
            ...
            m.viewColumn(IGEN.get(zone)).assign(loadPlusInterconnector, Functions.min);
        ...
        // Get old values of IGEN to calculate spill factors later on.
        for (Zone zone : zoneList) {
        ...
        for (int row = 0; row < m.rows(); row++) {
            ...
            // In case both countries have negative residual load (more IGEN
            // than load), set RLOAD to zero, and reduce IGEN to LOAD in
            ...
            if ((smallerResidual <= 0) && biggerResidual <= 0) {
                ...
                m.viewColumn(IGEN.get(zoneSmallerResidual)).set(row, m.get(row, LOADINZONE.get(zoneSmallerResidual)));
                m.viewColumn(IGEN.get(zoneBiggerResidual)).set(row, m.get(row, LOADINZONE.get(zoneBiggerResidual)));
                ...
                if (diffSandB > 0) {
                    ...
                    if (Math.abs(smallerResidual) < Math.abs(m.get(row, INTERCONNECTOR))) {
                        // Substract export IGEN from interconnector capacity,
                    ...
                    if (Math.abs(smallerResidual) < Math.abs(m.viewColumn(INTERCONNECTOR).get(row))) {
                        ...
                        m.set(row, RLOADINZONE.get(zoneBiggerResidual), 0);
                        m.set(row, RLOADINZONE.get(zoneSmallerResidual), 0);
                        m.set(row, IGEN.get(zoneSmallerResidual),
                                (m.get(row, IGEN.get(zoneSmallerResidual)) - biggerResidual));
                    } else {
                        ...
                        m.set(row, RLOADINZONE.get(zoneSmallerResidual), 0);
                        m.set(row, IGEN.get(zoneSmallerResidual),
                                (m.get(row, IGEN.get(zoneSmallerResidual)) - m.get(row, INTERCONNECTOR)));
                    }
                }
            }
        ...
        // per country.
        // TODO: This query gets YearlySegmentLoad as well.
