...
public class CreatingFinancialReports extends AbstractClearElectricitySpotMarketRole<DecarbonizationModel> {
    ...
    void createFinancialReportsForPowerPlantsAndTick(Iterable<PowerPlant> plants, long tick) {
        ...
        for (PowerPlant plant : plants) {
            ...
            // double totalSupply = plant.calculateElectricityOutputAtTime(tick,
            // false);
            // logger.warn(plant.getName());
            PpdpAnnual ppdp = reps.ppdpAnnualRepository.findPPDPAnnualforPlantForCurrentTick(plant, tick);
            // logger.warn(ppdp.getPowerPlant().getName());
            // financialPowerPlantReport.setProduction(totalSupply);
            double totalSupply;
            if (ppdp != null) {
                totalSupply = ppdp.getYearlySupply();
                // logger.warn("Total Supply" + totalSupply);
                // logger.warn("PPDP Status" + ppdp.getStatus());
            } else {
                totalSupply = 0;
            }
            financialPowerPlantReport.setProduction(totalSupply);
            ...
            // financialPowerPlantReport
            // .setFullLoadHours(reps.powerPlantRepository.calculateFullLoadHoursOfPowerPlant(plant,
            // tick));
            financialPowerPlantReport.setFullLoadHours(totalSupply / plant.getActualNominalCapacity());
