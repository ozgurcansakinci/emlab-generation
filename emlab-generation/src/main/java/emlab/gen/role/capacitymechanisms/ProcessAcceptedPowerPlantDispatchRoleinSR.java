 ...
 */
@RoleComponent
public class ProcessAcceptedPowerPlantDispatchRoleinSR extends AbstractRole<StrategicReserveOperator>
    ...
    public void act(StrategicReserveOperator strategicReserveOperator) {
        ...
        for (Segment segment : reps.segmentRepository.findAll()) {
            ...
            SegmentClearingPoint scp = reps.segmentClearingPointRepository
                    .findOneSegmentClearingPointForMarketSegmentAndTime(getCurrentTick(), segment, market, false);
            ...
            for (PowerPlantDispatchPlan plan : reps.powerPlantDispatchPlanRepository
                ...
                if (plan.getBiddingMarket().getNodeId().intValue() == market.getNodeId().intValue()) {
                    ...
                    if (plan.getStatus() >= 2) {
                        // logger.warn("Checking Accepted Bids finding accepted
                        // bids " +plan.getStatus());
                        if (plan.getSRstatus() <= -10) {
                            // logger.warn("Checking SR Status Contracted " +
                            // plan.getSRstatus());
                            double moneyReturned = ((plan.getAcceptedAmount() * scp.getPrice()
                                    * segment.getLengthInHours())
                            ...
                            // logger.warn("SRO "+
                            // strategicReserveOperator.getName() +" CASH
                            // Before" +strategicReserveOperator.getCash());
