...
import emlab.gen.role.market.DetermineAnnualResidualLoadCurvesRole;
...
import emlab.gen.role.operating.PayToStakeholderRole;
...
public class DecarbonizationModelRole extends AbstractRole<DecarbonizationModel>implements Role<DecarbonizationModel> {
    ...
    @Autowired
    private DetermineAnnualDemandGrowthRole determineAnnualDemandGrowthRole;
    @Autowired
    ...
    @Autowired
    private PayToStakeholderRole payToStakeholderRole;
    @Autowired
    ...
    @Autowired
    private DetermineAnnualResidualLoadCurvesRole determineAnnualResidualLoadCurve;
    @Autowired
    ...
    public void act(DecarbonizationModel model) {
        ...
        if ((getCurrentTick() >= 0) && (model.isSimpleCapacityMarketEnabled())) {
            timerMarket.reset();
            timerMarket.start();
            logger.warn(" 0b. Run Simple Capacity Market");
        ...
        logger.warn("  0c. Determining hourly demand growth");
        timerMarket.reset();
        timerMarket.start();
        for (ElectricitySpotMarket market : reps.marketRepository.findAllElectricitySpotMarketsAsList()) {
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        timerMarket.reset();
        timerMarket.start();
        logger.warn("  2b. Submitting offers to market");
        ...
        timerMarket.start();
        // if (getCurrentTick() >= 1) {
        logger.warn(" 4a. Determining residual load duration curve for investment role");
        determineAnnualResidualLoadCurve.act(model);
        // }
        timerMarket.stop();
        logger.warn("        took: {} seconds.", timerMarket.seconds());
         ...
         * Maintenance and CO2
         */
        logger.warn("  5a. Paying for maintenance & co2");
        timerMarket.reset();
        timerMarket.start();
        for (EnergyProducer producer : reps.genericRepository.findAllAtRandom(EnergyProducer.class)) {
            // do accounting
        ...
        logger.warn(" 5b. Paying for storage units");
        ...
        logger.warn(" 7a. Creating power plant financial reports.");
