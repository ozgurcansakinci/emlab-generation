...
public interface PowerPlantRepository extends GraphRepository<PowerPlant> {
    ...
    public Iterable<PowerPlant> findOperationalPowerPlantinMarketatTime(@Param("market") ElectricitySpotMarket market,
            @Param("tick") long tick);
    ...
    // @Query(value =
    // "g.v(market).as('x').in('INVESTOR_MARKET').has('type',T.neq,'emlab.gen.domain.agent.TargetInvestor').has('type',T.neq,'emlab.gen.domain.agent.StochasticTargetInvestor').back('x').out('ZONE').in('REGION').in('LOCATION').filter{it.__type__=='emlab.gen.domain.technology.PowerPlant'}.as('p').out('TECHNOLOGY').filter{it.intermittent
    // == false}.back('p').as('plant').filter{((it.constructionStartTime +
    // it.actualPermittime + it.actualLeadtime) <= tick) && (it.dismantleTime >
    // tick)}.sort{it.profitability}_()", type = QueryType.Gremlin)
    // public Iterable<PowerPlant>
    // findOperationalNonIntermittentPowerPlantsByAscendingProfitabilityAndMarket(
    // @Param("market") ElectricitySpotMarket market, @Param("tick") long tick);
    ////////////
    ...
    ////////////////////////////////////////////////////////////
    @Query(value = "t = new Table();"
            + "g.v(market).out('ZONE').in('REGION').in('LOCATION').filter{it.__type__=='emlab.gen.domain.technology.PowerPlant'}.filter{((it.constructionStartTime + it.actualPermittime + it.actualLeadtime) <= tick) && (it.dismantleTime > tick)}.as('pp').out('TECHNOLOGY').filter{it.intermittent == false}.as('ty').table(t){it.actualNominalCapacity}{it.baseSegmentDependentAvailability}.cap().next(); "
            + "capacitySum = 0; for (row in t){capacitySum += row.get(0) * row.get(1);}; return capacitySum;", type = QueryType.Gremlin)
