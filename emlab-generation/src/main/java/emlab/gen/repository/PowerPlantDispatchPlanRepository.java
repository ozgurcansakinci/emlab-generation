...
public interface PowerPlantDispatchPlanRepository extends GraphRepository<PowerPlantDispatchPlan> {
    ...
    // @Query(value =
    // "g.V.filter{it.getProperty('__type__')=='emlab.gen.domain.market.electricity.PowerPlantDispatchPlan'
    // && it.getProperty('time')==time}",
    ...
    // @Query(value =
    // "g.v(segment).in('SEGMENT_DISPATCHPLAN').propertyFilter('time',
    // FilterPipe.Filter.EQUAL, time)",
    // type = QueryType.Gremlin)
    // public Iterable<PowerPlantDispatchPlan>
    ...
    // @Query(value =
    // "g.v(segment).in('SEGMENT_DISPATCHPLAN').propertyFilter('time',
    // FilterPipe.Filter.EQUAL, time).sort{it.price}._()",
    // type = QueryType.Gremlin)
    // public Iterable<PowerPlantDispatchPlan>
    ...
    @Query(value = "sum=0;ppdps=g.v(producer).out('BIDDER').propertyFilter('time', FilterPipe.Filter.EQUAL, time).propertyFilter('status', FilterPipe.Filter.GREATER_THAN_EQUAL , 2).propertyFilter('forecast', FilterPipe.Filter.EQUAL, forecast);"
            ...
            // ppdp.getProperty('capacityLongTermContract');"+
            // "hoursInSegment =
            // ppdp.out('SEGMENT_DISPATCHPLAN').next().getProperty('lengthInHours');"+
            ...
            + "production = totalAmount * hoursInSegment;" + "sum = sum + production};"
            + " return sum;", type = QueryType.Gremlin)
    // >>>>>>> PCBhagwat/feature/mergingEconomicDismantlingAndCapacityMarkets2
    ...
    // (segment)<-[:SEGMENT_DISPATCHPLAN]-(ppdp)-[:BIDDINGMARKET]->(market)
    // WHERE (ppdp.time = {time}) and (ppdp.status >= 2) RETURN ppdp")
    // public Iterable<PowerPlantDispatchPlan>
    ...
    // @Query("START segment = node({segment} MATCH
    // (segment)<-[:SEGMENT_DISPATCHPLAN]-(ppdp)<-[:BIDDER]-(node({producer}))
    // WHERE (ppdp.time = {time}) AND (ppdp.status >=1) RETURN ppdp")
    // public Iterable<PowerPlantDispatchPlan>
