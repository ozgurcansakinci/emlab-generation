...
java -d64 -server -Xmx3072m -Drun.id=$runId -DSCENARIO_FOLDER=$SCENARIOPATH -Dresults.path=$LOCALRESULTFOLDER/$JOBNAME -Dscenario.file=$SCENARIO-$PBS_ARRAYID".xml" -Djava.library.path=/opt/ibm/ILOG/CPLEX_Studio1262/cplex/bin/x86-64_linux $QUERYCOMMAND -jar $LOCALJARFILE
