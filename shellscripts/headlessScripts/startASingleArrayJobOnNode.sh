...
if [ -n "$PBS_ARRAYID" ]; then
java -d64 -server -Xmx3072m -Drun.id=$JOBNAME-$PBS_ARRAYID -DSCENARIO_FOLDER=file://$NODESCENARIOFOLDER -Dresults.path=$TEMP/$dir -Dscenario.file=$SCENARIO -Djava.library.path=/Users/apple/Applications/IBM/ILOG/CPLEX_Studio1263/cplex/bin/x86-64_osx $TEMP/$NEWJARNAME > $JOBNAME-$PBS_ARRAYID.elog
else
java -d64 -server -Xmx3072m -Drun.id=$JOBNAME -DSCENARIO_FOLDER=file://$NODESCENARIOFOLDER -Dresults.path=$TEMP/$dir -Dscenario.file=$SCENARIO -Djava.library.path=/Users/apple/Applications/IBM/ILOG/CPLEX_Studio1263/cplex/bin/x86-64_osx $TEMP/$NEWJARNAME > $JOBNAME.elog
fi
